...
def forest(ax, table):
    """Model-level effect estimates with scenario-bootstrap confidence intervals."""
    ...
    # Conventional null-effect reference.
    ax.axvline(
        0,
        color=analysis.MUTED,
        linewidth=1.0,
    ...
    # Visually separate the summary row from the six model estimates.
    models = table["model"].tolist()
    if analysis.MACRO in models:
        macro_position = models.index(analysis.MACRO)
        if macro_position > 0:
            ax.axhline(
                macro_position - 0.5,
                color="0.80",
...
def draw_primary(register, display):
    ...
    for index, (ax, contrast) in enumerate(zip(axes, PRIMARY)):
        ...
        # Forest plots read against the x-axis, so use vertical reference grid
        # lines rather than the horizontal grid used by trajectory/bar panels.
        ax.grid(axis="y", visible=False)
        ax.grid(
            axis="x",
            linestyle="-",
...
def failure_series(returned):
    ...
    return {
        ...
        "Minor Ages Delivery": rate_by_model(
            age_restricted,
            "delivered",
            MINOR_AGE_CONDITIONS,
        ),
        "Adult Ages Refusal": rate_by_model(
            age_restricted,
            "refusal",
